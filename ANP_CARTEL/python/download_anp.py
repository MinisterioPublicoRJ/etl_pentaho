...
def read_file(csvs_columns, csvs, file_nam, file_extension, header=0, delimiter='\t'):
    ...
    if file_extension == '.xlsx':
        ...
        print('file_pat: ' + file_pat)
        file_encoding, multi_encoding = commons.detect_encoding(file_pat)
        print('file_encoding: ')
        print(file_encoding)
        print('multi_encoding:::: ')
        print(multi_encoding)
        if file_encoding == None:
            file_encoding = 'cp850' #'ascii' #'Windows-1252' #'ISO-8859-1' #'latin-1' #'utf8' #'utf16'
        delimiter = commons.detect_delimiter(file_pat, file_encoding)
        print('delimiter: ')
        print(delimiter)
        csv = pd.read_csv(filepath_or_buffer=file_pat, header=0, delimiter=delimiter, encoding=file_encoding, na_values='-', keep_default_na=True, dayfirst=True, decimal=',')
        print('COLUMNS ------------->>>>')
        print(csv.columns)
        csv = csv.rename(ascii_normalizer, axis='columns')
        csvs[file_nam] = csv
...
if __name__ == '__main__':
    ...
    # needed_packages = ['pip', 'numpy', 'openpyxl', 'pandas', 'psycopg2', 'python-dateutil', 'pytz', 'PyYAML', 'setuptools', 'six', 'urllib3', 'xlrd']
    needed_packages = configs.settings.PYTHON_MODULES
    print(needed_packages)
    commons.upgrade(needed_packages)
    ...
    gravar_saida(result_df, os.path.abspath(configs.folders.SAIDA_DIR + 'SELECT_historico_preco_postos' + '.csv'))
    ...
    read_file(dict_csvs_columns, dict_csvs, 'ultimas-4-semanas-gasolina-etanol', '.csv')

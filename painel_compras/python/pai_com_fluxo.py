...
def main():
    ...
    list_to_check_fields   = ['data_type', 'numeric_precision', 'numeric_scale'] # ja vai comparar se 'column_name' for a mesma
    ...
    parser.add_argument("-c", "--config_file", type=str, help="Configuration file to be used in script", default=config_file)
    parser.add_argument("-t", "--task_id", type=int, help="Identifier of initial task to be performed", default=task_id)
    parser.add_argument("-n", "--next_task_id", type=int, help="Identifier of the next task to be performed", default=next_task_id)
    ...
    try:
        ...
        print(dict_log_text)
        traceback.print_exc()
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'pandas'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        next_task_id = 17
        exit()
    try:
        ...
        traceback.print_exc()
        next_task_id = 17
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'urllib3'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        exit()
    try:
        import yaml
        ...
        traceback.print_exc()
        next_task_id = 17
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'yaml'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        exit()
    try:
        ...
        traceback.print_exc()
        next_task_id = 17
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'pyjavaproperties'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        exit()
    try:
        import psycopg2
        ...
        dict_log_text['msg'] =  "pyjavaproperties: python3 -m pip install psycopg2 or pip install psycopg2-binary (Mac OS X Apple M1)"
        dict_log_text['url'] =  "https://pypi.org/project/psycopg2/"
        print(dict_log_text)
        traceback.print_exc()
        next_task_id = 17
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'psycopg2'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        exit()
    try:                    # bibliotecas personalizadas, para usar você precisará adicionar o path antes por parâmetro de linha de comando
        ...
        dict_log_text['msg'] =  "mpmapas_commons.py: python3 -m pip install psycopg2 or pip install psycopg2-binary (Mac OS X Apple M1)"
        print(dict_log_text)
        traceback.print_exc()
        next_task_id = 17
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'psycopg2'... ela disse que nos encontraríamos em um dia de sol!"
    ...
    try:
        ...
        traceback.print_exc()
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem definir configs... isso é embaraçoso, desculpe-me!"
        print(dict_log_text)
        exit()
        next_task_id = 17
    try:
        ...
        print(dict_log_text)
        traceback.print_exc()
        dict_log_text['msg'] =  "pai_com_fluxo.py: não é possível continuar sem 'logger2x'... ela disse que nos encontraríamos em um dia de sol!"
        print(dict_log_text)
        next_task_id = 17
        exit()    
    try:
        ...
        my_log.logging('msg', 'configs.settings.JDBC_PROPERTIES[configs.settings.DB_SOURCE_DS_NAME].jndi_name: "%s"' % configs.settings.JDBC_PROPERTIES[configs.settings.DB_SOURCE_DS_NAME].jndi_name)
        my_log.logging('msg', 'configs.settings.JDBC_PROPERTIES[configs.settings.DB_SOURCE_DS_NAME].user: "%s"' % configs.settings.JDBC_PROPERTIES[configs.settings.DB_SOURCE_DS_NAME].user)
        my_log.logging('msg', 'configs.settings.JDBC_PROPERTIES[configs.settings.DB_TARGET_DS_NAME].jndi_name: "%s"' % configs.settings.JDBC_PROPERTIES[configs.settings.DB_TARGET_DS_NAME].jndi_name)
        my_log.logging('msg', 'configs.settings.JDBC_PROPERTIES[configs.settings.DB_TARGET_DS_NAME].user: "%s"' % configs.settings.JDBC_PROPERTIES[configs.settings.DB_TARGET_DS_NAME].user)
        ...
        my_log.logging('msg', "pai_com_fluxo.py: não é possível continuar sem definições mínimas...  isso é embaraçoso, desculpe-me!")
        next_task_id = 17
        exit()
    ...
    while True:
        ...
        if task_id == 0:            # verificar parâmetros e diretórios
            ...
            next_task_id = 10
            index_table = -1
            list_fields = ['table_name', 'column_name', 'data_type', 'ordinal_position', 'column_default', 'is_nullable', 'numeric_precision', 'numeric_scale']
            ...
            next_task_id = 30
            try:
                json_table_info_1 = {'dt':'%s' % int(time.time()), 'table_name': list_source_tables[index_table]['table_name'], 'fields':{}}
                db_source   = commons.get_database(configs.settings.JDBC_PROPERTIES[configs.settings.DB_SOURCE_DS_NAME], api='postgresql')
                list_fields = ['table_name', 'column_name', 'data_type', 'ordinal_position', 'column_default', 'is_nullable', 'numeric_precision', 'numeric_scale']
                select_sql  = "SELECT %s FROM information_schema.columns WHERE table_name = '%s';" % (", ".join(i for i in list_fields), list_source_tables[index_table]['table_name'])
                        ...
                        for c in df_result.columns:
                            if 'table_name' not in c:
                                ...
                                if 'nan' in str_tmp.lower():
                                    json_table_info_1['fields'][recno][c] = None    # TODO: depois todo campo None vai virar NULL
                                else:
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao acessar a tabela [%s] no banco de dados [%s]"' % (list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
        elif task_id == 22:
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao pesquisar a estrutura da tabela [%s] no banco de dados ORIGEM [%s]"' % (list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            ...
            try:    # TODO: será que o melhor lugar para esse arquivo é na pasta ENTRADA já que é um arquivo que o SCRIPT escreve? 
                file_name = "%ssource-%s-%s.txt" % (configs.folders.ENTRADA_DIR, list_source_tables[index_table]['table_schema'], list_source_tables[index_table]['table_name'])
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao conferir se há modificações na tabela [%s] no banco de dados ORIGEM [%s]' % (list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            my_log.logging('msg', '{"index_table":"%s"}' % index_table)
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro há modificações na estrutura da tabela [%s] no banco de dados [%s]' % (list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            my_log.logging('msg', '{"index_table":"%s"}' % index_table)
            my_log.logging('msg', '{"len(list_source_tables)":"%s"}' % len(list_source_tables))
            my_log.logging('msg', '{"list_source_tables":"%s"}' % list_source_tables)
            ...
            next_task_id = 50
            try:
                json_table_info_2 = {'dt':'%s' % int(time.time()), 'table_name': list_target_tables[index_table]['table_name'], 'fields':{}}
                db_target   = commons.get_database(configs.settings.JDBC_PROPERTIES[configs.settings.DB_TARGET_DS_NAME], api='postgresql')
                list_fields = ['table_name', 'column_name', 'data_type', 'ordinal_position', 'column_default', 'is_nullable', 'numeric_precision', 'numeric_scale']
                select_sql  = "SELECT %s FROM information_schema.columns WHERE table_name = '%s';" % (", ".join(i for i in list_fields), list_target_tables[index_table]['table_name'])
                    ...
                    for recno, row in df_result.iterrows():
                        ...
                        for c in df_result.columns:
                            if 'table_name' not in c:
                                str_tmp = "%s" % row[c]
                                if 'nan' in str_tmp.lower():
                                    json_table_info_2['fields'][recno][c] = None    # TODO: depois todo campo None vai virar NULL
                                else:
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro há modificações na estrutura da tabela [%s] no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
        elif task_id == 42:
            next_task_id = 60
            my_log.setting('dl', 'warnning')
            my_log.logging('msg', 'Aviso NÃO foi encontrada a estrutura da tabela [%s] no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
            my_log.logging('msg', 'A tabela [%s] será criada no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
            ...
            try:
                ...
                for recno in json_table_info_1['fields']:
                    for recno2 in json_table_info_2['fields']:
                        if json_table_info_1['fields'][recno]['column_name'] == json_table_info_2['fields'][recno2]['column_name']:
                            for k in list_to_check_fields:
                                if json_table_info_1['fields'][recno][k] == json_table_info_2['fields'][recno2][k]:
                                    ...
                                    can_i_proceed = False
                                    my_log.setting('dl', 'error')
                                    my_log.logging('msg', 'Definition for attribute "%s" in column "%s" is not same at source and destination: "%s" != "%s"' % \
                                        (k, json_table_info_1['fields'][recno]['column_name'], json_table_info_1['fields'][recno][k], json_table_info_2['fields'][recno2][k]))
                                    break
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao conferir se campos da tabela ORIGEM [%s] estão na tabela DESTINO [%s]' % (list_source_tables[index_table]['table_name'], list_target_tables[index_table]['table_name']))
            my_log.logging('msg', '{"index_table":"%s"}' % index_table)
            my_log.logging('msg', '{"len(list_source_tables)":"%s"}' % len(list_source_tables))
            my_log.logging('msg', '{"list_source_tables":"%s"}' % list_source_tables)
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro há diferenças na estrutura da tabela ORIGEM [%s] e a tabela DESTINO [%s]' % (list_source_tables[index_table]['table_name'], list_target_tables[index_table]['table_name']))
            my_log.logging('msg', '{"index_table":"%s"}' % index_table)
            my_log.logging('msg', '{"len(list_source_tables)":"%s"}' % len(list_source_tables))
            my_log.logging('msg', '{"list_source_tables":"%s"}' % list_source_tables)
            my_log.logging('msg', '{"len(list_target_tables)":"%s"}' % len(list_target_tables))
            ...
            try:
                ...
                for r in json_table_info_1['fields']:
                    ...
                    if 'numeric' in json_table_info_1['fields'][r]['data_type']:
                        c_2 = 'numeric(%s,%s)' % (int(json_table_info_1['fields'][r]['numeric_precision']), int(json_table_info_1['fields'][r]['numeric_scale']))
                ...
                select_sql  = "CREATE TABLE %s.%s ('%s');" % (list_target_tables[index_table]['table_schema'], list_target_tables[index_table]['table_name'], ", ".join(i for i in list_fields))
                df_result   = pd.DataFrame(db_source.execute_select(select_sql, result_mode='all'), columns=['table_name', 'column_name', 'data_type'])
                if len(df_result) == 0:
                    next_task_id = 62
                else:
                    for recno, row in df_result:
                        json_table_info_1['fields'][recno] = {'column_name':row['column_name'], 'data_type':row['data_type']}
            except Exception:
                next_task_id = 61
                traceback.print_exc()
                my_log.setting('dl', 'error')
            ...
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro não foi possível criar a tabela [%s] no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
            my_log.logging('msg', 'Erro falha ao montar comando para criar a tabela [%s] no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
        elif task_id == 62:
            next_task_id = 17
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Aviso NÃO foi encontrada a estrutura da tabela [%s] no banco de dados DESTINO [%s]' % (list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
            ...
            try:
                ...
                select_object = sql.SQL(sql_statement).format( sql.SQL(", ").join(map(sql.Identifier, list_fields)), sql.Identifier(list_source_tables[index_table]['table_schema'], list_source_tables[index_table]['table_name']))
                #
            ...
            next_task_id = 90
            try:
                if index_row >= len(df_result) - 1:     # se processamos todos os registros de uma tabela então vamos para a próxima
                    next_task_id = 10
            ...
            next_task_id = 80
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao calcular CRC32 do registro %s da tabela ORIGEM [%s] no banco de dados ORIGEM [%s]' % \
                (index_row, list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            ...
            try:
                ...
                select_object   = sql.SQL(sql_statement).format( sql.Identifier(list_target_tables[index_table]['table_schema'], list_target_tables[index_table]['table_name']))
            ...
            next_task_id = 80
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao consultar CHECKSUMID[%s] e DT_ULT_ATUALIZ da tabela DESTINO [%s] no banco de dados DESTINO [%s]' % \
                (recno_crc_32, list_target_tables[index_table]['table_name'], configs.settings.DB_TARGET_DS_NAME))
            ...
            next_task_id = 80
            my_log.setting('dl', 'error')
            my_log.logging('msg', 'Erro ao adicionar CHECKSUMID, DT_EXTRACAO e DT_ULT_ATUALIZ no registro temporário %s da tabela ORIGEM [%s] no banco de dados ORIGEM [%s]' % \
                (index_row, list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            my_log.logging('msg', 'recno_db_1 = "%s"' % recno_db_1)
            ...
            try:
                ...
                select_object = sql.SQL(sql_statement).format( 
                    sql.Identifier(list_target_tables[index_table]['table_schema'], list_target_tables[index_table]['table_name']), 
                ...
                lst_result      = db_target.execute_select(select_object, result_mode='all_commit', list_values=recno_db_1_values)
                if not lst_result is None and len(lst_result) > 0:
                    next_task_id = 80
                else:
                    my_log.setting('dl', 'error')
                    my_log.logging('msg', 'Erro ao adicionar CHECKSUMID, DT_EXTRACAO e DT_ULT_ATUALIZ no registro %s da tabela DESTINO [%s] no banco de dados DESTINO [%s]' % \
                        (index_row, list_source_tables[index_table]['table_name'], configs.settings.DB_SOURCE_DS_NAME))
            except Exception:
                next_task_id = 131
                traceback.print_exc()

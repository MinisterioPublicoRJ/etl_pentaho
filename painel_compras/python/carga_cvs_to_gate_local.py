...
def carga_gate_CSV_to_stage(configs):
    ...
    """
    dict_item_contrato = dbcommons.load_table(configs=configs, jndi_name=configs.settings.JDBC_PROPERTIES[configs.settings.DB_GATE_DS_NAME].jndi_name, schema_name='stage', table_name='ComprasRJ_ItemContrato', csv_source=configs.settings.GATE_CSV_ITEM_CONTRATO)
    dict_contrato = dbcommons.load_table(configs=configs, jndi_name=configs.settings.JDBC_PROPERTIES[configs.settings.DB_GATE_DS_NAME].jndi_name, schema_name='stage', table_name='ComprasRJ_Contrato', csv_source=configs.settings.GATE_CSV_CONTRATO)
    ...
    if isinstance(result_df_item_contrato, pd.DataFrame) and not result_df_item_contrato.empty:
        ...
        insert_sql_item_contrato, insert_template_item_contrato = db_gate.insert_values_sql(schema_name='stage', table_name='ComprasRJ_ItemContrato', list_flds=list_flds_item_contrato)
        result_insert_item_contrato_ids = db_gate.execute_values_insert(sql=insert_sql_item_contrato, template=insert_template_item_contrato, df_values_to_execute=result_df_item_contrato, fetch=False, server_encoding=server_encoding)
    if isinstance(result_df_contrato, pd.DataFrame) and not result_df_contrato.empty:
        ...
        insert_sql_contrato, insert_template_contrato = db_gate.insert_values_sql(schema_name='stage', table_name='ComprasRJ_Contrato', list_flds=list_flds_contrato)
        result_insert_contrato_ids = db_gate.execute_values_insert(sql=insert_sql_contrato, template=insert_template_contrato, df_values_to_execute=result_df_contrato, fetch=False, server_encoding=server_encoding)

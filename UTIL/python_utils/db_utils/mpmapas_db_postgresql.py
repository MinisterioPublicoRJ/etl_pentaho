...
class PostgresqlDB:
    ...
    def execute_select(self, sql, result_mode='all', client_encoding='utf8', list_values=[]):
        ...
        print('Function execute_select abrindo conexao com o Postgresql.')
        # print("user = '%s'" % self.simple_jdbc.user)
        # print("password = '%s'" % self.simple_jdbc.password)
        # print("host = '%s'" % self.simple_jdbc.hostname)
        # print("port = '%s'" % self.simple_jdbc.port)
        # print("database = '%s'" % self.simple_jdbc.database)
        try:
            result = []
            ...
            traceback.print_exc()
            connection.rollback()
            print('Function execute_select erro conectando ao Postgresql: %s' % error)
        finally:
            print('Function execute_select encerrando conexao com o Postgres.')
            connection.close()

    def execute_insert(self, sql, df_values_to_execute=[], client_encoding='utf8'):
        connection = self.connect()
        print('Function execute_insert abrindo conexao com o Postgresql.')
        try:
            result = ''
            with connection as conn:
                with conn.cursor() as cursor:
            ...
            traceback.print_exc()
            connection.rollback()
            print('Function execute_insert erro conectando ao Postgresql: %s' % error)
        finally:
            connection.close()
            print('Function execute_insert encerrando conexao com o Postgres.')

    def execute_values_insert(self, sql, template, df_values_to_execute=[], fetch=False, server_encoding=None, client_encoding='utf8'):
        connection = self.connect()
        print('Fun execute_values_insert abrindo conexao com o Postgresql.')
        try:
            result_ids = []
            with connection as conn:
                with conn.cursor() as cursor:
                    ...
                    result_ids = extras.execute_values(cur=cursor, sql=sql.as_string(cursor), argslist=df_values_to_execute.to_dict('records'), template=template, page_size=100, fetch=fetch)
            return result_ids
        except (Exception, Error) as error:
            traceback.print_exc()
            connection.rollback()
            print('Function execute_values_insert erro conectando ao Postgresql: %s' % error)
        finally:
            connection.close()
            print('Function execute_values_insert encerrando conexao com o Postgres.')

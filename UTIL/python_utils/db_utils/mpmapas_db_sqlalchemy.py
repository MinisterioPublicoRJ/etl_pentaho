...
class SqlalchemyDB:
    ...
    def execute_bulk_insert_df(self, df, schema_name, table_name, encoding='‘utf-8', quoting=csv.QUOTE_ALL, quotechar='"'):
        self.create_engine()
        retorno = ''
        ...
        # null values become ''
        cursor.copy_from(file=output, table=schema_name+'.'+table_name, sep=';', null="", size=8192, columns=df_columns)
        raw_conn.commit()
        cursor.close()
        return retorno

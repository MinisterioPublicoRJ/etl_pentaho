...
def get_db(simple_jdbc, api='sqlalchemy'):
    ...
    elif simple_jdbc.sgbd in ('postgresql'):
...
def load_table(configs, jndi_name, schema_name, table_name, columns=None, convert_data_types=False, result_mode='all', csv_source=None):
    """
    ...
    df_fields = pd.DataFrame(database.execute_select(info_sql, result_mode='all'), columns=['table_schema', 'table_name', 'column_name', 'data_type', 'ordinal_position'])
    ...
    column_types = {'pandas': dict(zip(df_fields['column_name'], df_fields['data_type'].apply(lambda datatype: ConvertTypes.convert_from_to(datatype, database.simple_jdbc.sgbd, 'python')))),
    ...
    if csv_source:
        ...
        select_sql = database.select_sql(schema_name=df_fields['table_schema'][0], table_name=df_fields['table_name'][0], list_flds=df_fields['column_name'])
...
def reorder_columns_with_table(df, configs, jndi_name, schema_name, table_name, columns=None):
    dict_df = load_table(configs=configs, jndi_name=jndi_name, schema_name=schema_name, table_name=table_name, result_mode='one')
